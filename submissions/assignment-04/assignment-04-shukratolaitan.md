...
Docker already provides user-defined bridge networks, embedded DNS (127.0.0.1) & service-name resolution.
...
The Docker primitive responsible for this isolation is the user-defined bridge network. To allow the stranger container to reach the DB while still running on cohort-other, I could connect it to cohort-net as well:
...
No the named volume cohort-db-data persists.
...
The data is memory-backed and not persisted to disk.
The data disappears automatically when the container stops.
